...
def gather_nd(params, indices):
    ...
    rets = []
    for _i in range(indices.size()[0]):
        for _j in range(indices.size()[1]):
            rets.append(params[indices[_i][_j][0], indices[_i][_j][1]])
    rets = torch.stack(rets, dim=0)
    rets = rets.reshape(indices.size()[:-1] + params.size()[2:])
    return rets
...
def log_prob_from_logits(logits):
    return logits - torch.log(torch.exp(logits).sum(dim=-1, keepdim=True))
    # return logits - torch.reduce_logsumexp(logits, axis=-1, keepdims=True)
...
def beam_search(
    ...
    def grow_finished(
        ...
        # negative values
        curr_scores += (1.0 - curr_finished.float()) * -INF
    ...
    def grow_alive(
        ...
        # values
        curr_scores += curr_finished.float() * -INF
        return compute_topk_scores_and_seq(
            curr_seq,
            curr_scores,
            curr_log_probs,
    ...
    def grow_topk(
        ...
        # possibilites
        flat_curr_scores = torch.reshape(
